...
| 2026-06-03 | pitch     | Mushee (B) | #2     | Cover `.s-cover{display:grid}` overridden by `.slide.active{display:flex}` (higher specificity) → two-column cover stacked vertically | Layout Reviewer | Cover grid rule must out-specify `.slide.active`: use `.slide.s-cover.active{display:grid}` (not bare `.s-cover`) |
| 2026-06-03 | pitch     | Mushee (B) | #2     | Inner grid with `height:100%` placed below eyebrow+h2 overflowed canvas, clipping a stacked photo below the fold | Layout Reviewer | A content grid that is NOT the only flex child of `.slide` must not use `height:100%`; bound it explicitly or use `flex:1;min-height:0` so siblings above are accounted for |
| 2026-06-03 | pitch     | Mushee (B) | #2     | Footnote (`bottom:26px;left:96px`) collided with nav-counter (`bottom:26px;left:40px`) → ran together as one string | Layout Reviewer | Footnote must clear the nav row: raise to `bottom:~60px` and cap `right` so it never reaches the arrow cluster |
| 2026-06-03 | pitch     | Mushee (B) | #2     | Demo slide "Powered by FluidDocs" mark (`bottom:24px;right:40px`) overlapped nav arrows (same corner) | Layout Reviewer | Attribution mark on demo slide sits ABOVE the arrow cluster (`bottom:~78px`), not in the shared bottom-right nav zone |
| 2026-06-03 | pitch     | Mushee (B) | (brand) | Guessed Unsplash placeholder ID resolved to a wrong subject (potatoes on a mushroom-meat product slide) | Brand Reviewer | Verify each placeholder photo's actual subject by rendering; for on-theme stock, resolve the real `images.unsplash.com/photo-…` URL via the photo page rather than guessing numeric IDs |
