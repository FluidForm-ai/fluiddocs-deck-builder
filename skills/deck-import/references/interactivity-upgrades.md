...
When a slide carries an interactive product demo (per the Phase 5 upgrade menu, or because the user opted into `demo_mode: interactive` in Phase 2), add a small badge on the slide that telegraphs "this is interactive" to a viewer who hasn't been told. The badge is the difference between a viewer poking around and a viewer not noticing the demo is live.

Keep it restrained, the goal is "this is alive", not "look at me".

### On-brand badge recipe

```html
<div class="demo-badge">
  <span class="demo-dot"></span>
  Interactive · Click anywhere
</div>
```
...
- **White background, primary-color border + text, single pulsing dot.** Uses brand colors only. No invented accent colors.
- **11.5px font, 7px x 14px padding, 20px border-radius.** Smaller than a clickable button (this isn't a CTA), larger than legalese. The pill shape says "label" not "button."
- **Single pulsing dot, not the whole badge.** Whole-badge animation reads as urgent; single-dot pulse reads as "this is alive." Same signal, dramatically less visual weight.
- **`ease-in-out` over `linear`.** The pulse should feel like a heartbeat, not a metronome.
- **`absolute` top-right corner.** Out of the slide's main reading path. The eye finds it after the headline; the headline doesn't compete with it.

### What NOT to do

- Don't tilt the badge. Tilted UI elements read as decorative stickers, which clash with the rest of the deck.
- Don't use accent colors that aren't already in the deck's brand. The badge should look like the deck designed it, not like an external annotation.
- Don't animate position (`translate`, `translateY`). Movement attracts the eye away from the slide content.
- Don't add a chevron, arrow, or other "click me" indicator beyond the dot. The text "Interactive · Click anywhere" is plenty.

### When to add it

Add to any slide that has a working demo (`demo_mode: interactive` in the conversion brief, or a post-release demo-upgrade run). One badge per slide; don't repeat it on multiple slides if the demo spans more than one. Add it AFTER the demo itself is verified working, the badge advertises a capability, so verifying it's actually there first is non-negotiable.
